...
var OAuthAdapter = function(pConsumerSecret, pConsumerKey, pSignatureMethod) {
    ...
    function authorizeUICallback(e) {
        if (-1 !== e.url.indexOf("linkedin")) {
            var response = e.source.evalJS('(p = document.getElementsByClassName("access-code")) && p[0].innerHTML');
            response ? (pin = response, destroyAuthorizeUI(), receivePinCallback()) : (loadingView && loadingView.hide(), 
            loadingContainer && loadingContainer.hide(), webView && webView.show()), loading = !1, 
            clearInterval(intervalID), estimates[estimateID] = new Date().getTime() - startTime, 
            Ti.App.Properties.setString("Social-LoadingEstimates", JSON.stringify(estimates));
        } else if (-1 !== e.url.indexOf("twitter")) {
    ...
    this.sendTwitterImage = function(options) {
        ...
        Ti.Android || client.setRequestHeader("Content-Type", "multipart/form-data");
    ...
    this.showLoadingUI = function() {
        ...
        client = Ti.Network.createHTTPClient({
            onload: function() {
                200 == client.status || 201 == client.status ? pSuccessMessage && pSuccessMessage(this.responseText) : pErrorMessage && pErrorMessage(this.responseText);
            },
            onerror: function() {
                Ti.API.error("Social.js: FAILED to send a request!");
                Ti.API.error(this.responseText);
