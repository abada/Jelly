...
//Rare exceptions to indicate a bug in the _educore code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_educore = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_educore]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.educore.module"
